...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!resendKey) {
    // Нет ключа → всё равно подтверждаем, payload уже в логе
    return res.status(200).json({ success: true, mailed: false });
  }
  ...
  try {
    ...
    if (!r.ok) {
      console.error('[coaches-inquiry] resend failed', r.status, await r.text());
      return res.status(200).json({ success: true, mailed: false });
    }
